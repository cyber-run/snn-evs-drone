...
class EventEncoder:
    ...
    def __init__(
        ...
        self._frame_stride = 2 * height * width   # precomputed flat-index stride
    ...
    def encode(self, events: np.ndarray,
        ...
        if len(events) == 0:
            ...
            return torch.zeros(n_bins, 2, self.height, self.width)
    ...
    def _encode_columns(
        ...
        if len(ts) > 0:
            ...
            xs_c = np.clip(xs, 0, self.width - 1)
            ys_c = np.clip(ys, 0, self.height - 1)

            flat_idx = (bin_idx * self._frame_stride
                        + ps * (self.height * self.width)
                        + ys_c * self.width + xs_c)

            counts = np.bincount(flat_idx, minlength=total)
        else:
            counts = np.zeros(total, dtype=np.intp)

        frames = counts.reshape(n_bins, 2, self.height, self.width).astype(np.float32)
